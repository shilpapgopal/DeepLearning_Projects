...
# no gradients needed
#features = []
final_feature_tensor = torch.tensor([])
final_feature_tensor = final_feature_tensor.to(device)
for images in train_loader:
    images = images.to(device)
    features_ret = model.forward(images)
    #features.append(features_ret)
    final_feature_tensor =  torch.cat((final_feature_tensor, features_ret), 0)
...
#print("final_feature_tensor.shape--", final_feature_tensor.shape)


# save features
torch.save(final_feature_tensor, "features.pt")
